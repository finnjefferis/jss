    ...
    <main className="min-h-screen bg-zinc-50 dark:bg-zinc-950 transition-colors">
      ...
      <section className="relative py-16 md:py-24 overflow-hidden">
        <div className="pointer-events-none absolute inset-0">
          <div className="absolute top-1/4 right-1/4 w-96 h-96 bg-coral-500/8 dark:bg-coral-500/5 rounded-full blur-3xl" />
        ...
        <div className="mx-auto max-w-6xl px-5 md:px-8 relative z-10">
          <div className="max-w-3xl">
            ...
            <a
              ...
              className="inline-flex items-center gap-2 rounded-xl bg-coral-600 px-8 py-4 text-sm font-bold text-white shadow-lg shadow-coral-600/25 hover:bg-coral-700 hover:scale-[1.02] transition-all"
            >
          ...
          <div className="lg:grid lg:grid-cols-[280px_1fr] lg:gap-16">
            ...
            <div className="mt-10 lg:mt-0 grid grid-cols-1 md:grid-cols-2 gap-6">
              {PROJECTS.map((project) => (
                <Link key={project.title} href={project.href} className="group block rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 overflow-hidden hover:-translate-y-1 hover:shadow-xl hover:shadow-coral-500/10 transition-all">
                  {project.type === "image" ? (
                    ...
                    <div className="relative aspect-[16/9] w-full overflow-hidden bg-zinc-900 dark:bg-zinc-950 flex flex-col justify-between p-6">
                      <div className="pointer-events-none absolute inset-0">
                        <div className="absolute top-0 right-0 w-64 h-64 bg-coral-600/15 rounded-full blur-3xl" />
                        <div className="absolute bottom-0 left-0 w-48 h-48 bg-pink-600/10 rounded-full blur-2xl" />
                      </div>
                      <div className="relative flex flex-wrap gap-2">
                        {project.tags.map((tag) => (
                          <span key={tag} className="px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider bg-coral-950/80 text-coral-300 border border-coral-800">
                            {tag}
                          </span>
                        ))}
                      </div>
                  ...
                  <div className="p-6">
                    <div className="flex items-center justify-between mb-1">
                      <p className="text-[10px] font-bold uppercase tracking-wider text-coral-600 dark:text-coral-400">{project.role}</p>
                    ...
                    <h3 className="text-lg font-bold text-zinc-900 dark:text-zinc-100 mb-2 group-hover:text-coral-600 transition-colors">{project.title}</h3>
      ...
      <section className="py-16 md:py-20 border-t border-zinc-100 dark:border-zinc-900">
        <div className="mx-auto max-w-6xl px-5 md:px-8">
          <div className="rounded-2xl border border-coral-100 dark:border-coral-900 bg-coral-50/50 dark:bg-coral-950/30 p-10 md:p-14 text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              ...
              <Link
                href="/#recent-work"
                className="inline-flex items-center justify-center gap-2 rounded-xl border border-zinc-200 dark:border-zinc-700 px-8 py-4 text-sm font-bold text-zinc-600 dark:text-zinc-400 hover:border-coral-300 hover:text-coral-600 transition-all"
              >
                See all projects
              </Link>
            </div>
